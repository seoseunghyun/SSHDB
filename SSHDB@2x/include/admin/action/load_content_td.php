...
<?php
 ...
 $value_b = $sshdb_get[$_GET['db']][$_GET['tb']][$content[$key]['id']];
    ...
    ?><div id="table_ele_<?=$content[$key]['id']?>:<?=$keys?>" class="table_tds table_td_key_<?=$keys?> table_td_id_<?=$content[$key]['id']?> table_td_<?=$td_bgcolor?> " width="<?=$width_a[$keys]?>"  ><?=$width_a[$keys]?><?php if($keys=='id'&&strlen($values)>10){
    if($keys==$_GET['attr']){ ?>
   ...
   <?php }else{
    ...
    <?php }else{ ?><?=htmlspecialchars(mb_substr($values,0, 30,'UTF-8')).'...'?><?php }
      }else{
      if($keys==$_GET['attr']){
     ?>
     <?=str_replace($_GET['search'],'<font color="#5bc90a">'.$_GET['search'].'</font>',htmlspecialchars($values))?>
     <?php }else{
	     ?><?=htmlspecialchars($values)?><?php
     } }?></td><?php
   
    }?><td class="table_td_id_<?=$content[$key]['id']?> table_td_<?=$td_bgcolor?> table_td">&nbsp;&nbsp;</tr></tr></div><?php
    }
   ?>