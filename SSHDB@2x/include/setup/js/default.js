...
$(document).ready(function(){var sshdbs_action='include/setup/action/';var content_wrap_now=1;if('devicePixelRatio'in window&&window.devicePixelRatio==2){$('.presol').each(function(){$(this).attr('src',$(this).attr('src').replace('.gif','@2x.gif'));});$('.presol_bg').each(function(){$(this).css({'background-image':$(this).css('background-image').replace('.gif','@2x.gif'),'background-size':'69px 23px'});});}
